...
- 프로필 사진 (닉네임 기준으로 저장, 안 설정하면 이니셜 원형 배지)
...
> 알려진 이슈: 소켓 연결 타이밍 관련 경쟁 상태로 가끔(1/3~1/4 확률) 특정 테스트가 랜덤하게 타임아웃으로 실패합니다. 재현 결과 애플리케이션 로직 문제는 아니고 테스트 하네스 쪽 문제로 보이며, 아직 근본 원인은 못 찾았습니다. 실패하면 `npm test`를 한 번 더 돌려보세요.
