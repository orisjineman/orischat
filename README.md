...
- "입력 중..." 표시
- 이모티콘 피커
- 커스텀 스티커 (`public/stickers/`에 이미지만 넣으면 자동으로 추가됨)
- 라이트 / 다크 모드 자동 대응 UI
...
`public/stickers/` 폴더에 이미지 파일(`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`)을 넣으면 자동으로 스티커 목록에 나타납니다. 서버를 재시작할 필요 없이, 파일을 넣고 채팅창에서 이모티콘 버튼(😊)을 눌러보면 바로 보입니다.
