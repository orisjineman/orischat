...
function createEmitters(io) {
  ...
  // 방의 고정 메시지 목록(최근 고정순)을 방 전체에 보냄
  async function broadcastPins(room) {
    try {
      io.to(room).emit('pins', await pinStore.listPins(room));
    } catch (err) {
      console.error('고정 메시지 전송 오류:', err);
    }
  }

  return { emitPersonalized, broadcastUserList, broadcastReadUpdate, broadcastPins };
}
