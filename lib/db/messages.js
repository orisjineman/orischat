...
// fromTime 이상 ~ beforeTime 미만 메시지를 (beforeTime에 가까운 쪽부터 최대 limit개) 시간순으로 반환.
// "검색 결과로 이동"할 때 이미 화면에 있는 가장 오래된 메시지까지 빈틈없이 이어 붙이려는 용도라서,
// 너무 많으면 오래된 쪽이 잘림(호출하는 쪽에서 개수로 판단).
const getMessagesBetween = whenEnabled(
  () => [],
  async (room, fromTime, beforeTime, limit = 500) => {
    const rows = await query(
      `SELECT ${MESSAGE_COLUMNS} FROM messages WHERE room = ? AND time >= ? AND time < ? ORDER BY time DESC LIMIT ?`,
      [room, fromTime, beforeTime, limit]
    );
    return hydrate(rows.reverse());
  }
);

// 메시지 하나(고정할 때 원본 확인용). 없으면 null
const getMessageById = whenEnabled(
  () => null,
  async (id) => {
    const [row] = await query('SELECT id, type, content, nickname, room, time FROM messages WHERE id = ?', [id]);
    return row || null;
  }
);

// 대화 내보내기용: 방의 최근 메시지 최대 limit개를 시간순으로. 사진 본문(base64)은 너무 커서
// 빼고, 리액션/작성자 식별자(clientId)도 싣지 않음.
const getMessagesForExport = whenEnabled(
  () => [],
  async (room, limit = 5000) => {
    const rows = await query(
      `SELECT id, type, CASE WHEN type = 'image' THEN '' ELSE content END as content, nickname, time,
              reply_to as replyTo, edited_at as editedAt
       FROM messages WHERE room = ? ORDER BY time DESC LIMIT ?`,
      [room, limit]
    );
    return rows.reverse().map(parseReplyTo);
  }
);
...
const cleanupOldMessages = whenEnabled(
  ...
  async () => {
    ...
    // 고정된 메시지는 남겨둠
    const old = await query('SELECT id FROM messages WHERE time < ? AND id NOT IN (SELECT message_id FROM pins)', [cutoff]);
