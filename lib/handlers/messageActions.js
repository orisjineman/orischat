...
function register(socket, { io, emit }) {
  ...
  socket.on('delete-message', async ({ messageId } = {}) => {
    ...
    // 고정돼 있던 메시지면 고정도 함께 풀림
    try {
      if (await pinStore.removePin(room, messageId)) await emit.broadcastPins(room);
    } catch (err) {
  ...
  socket.on('edit-message', async ({ messageId, content } = {}) => {
    ...
    try {
      ...
      if (await pinStore.updatePinPreview(room, messageId, previewOf('text', newContent))) await emit.broadcastPins(room);
    } catch (err) {
